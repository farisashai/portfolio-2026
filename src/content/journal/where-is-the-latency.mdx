...
This is a collection of my learnings and investigations in performance benchmarking. At its core, latency breaks down into two things: **compute time** and **data transfer time**. Data is always being transferred somewhere, but for this study I'm scoping it to transfer between two machines at a minimum—not process-to-process communication, not RAM access patterns, not CPU cache hierarchies. The interesting problems live in the network: the space between machines where time is spent and where most of it is wasted.
...
The point isn't that language choice doesn't matter. It's that **raw compute for typical operations is almost never the bottleneck**. Even in a high-level language, the CPU finishes its work in a fraction of the time it takes a packet to cross a network. The moment you're doing anything involving networking—an API call, a database query, a DNS lookup—compute becomes a rounding error next to data transfer. The optimization target shifts from "make the CPU faster" to "minimize the number of network hops and the distance each hop travels."
...
For a product, this means **optimizing a single action below the 10 ms window is almost never worth it**. If your API responds in 8 ms instead of 3 ms, no user will ever notice. The returns on micro-optimization for individual operations are invisible.

The exception is **high-volume compounding operations**—things like batch processing, streaming data pipelines, or tight loops that execute thousands of times per request. A 1 ms improvement that compounds across 500 iterations is a 500 ms win. Optimize there. For single user-facing actions, respect the human speed limit and spend your engineering time on the hops that actually matter.
