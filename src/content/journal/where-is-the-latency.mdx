...
This is a collection of my learnings and investigations in performance benchmarking. At its core, latency breaks down into two things: **compute time** and **data transfer time**. Data is always being transferred somewhere, but for this study I'm scoping it to transfer between two machines at a minimum, excluding process-to-process communication, RAM access patterns, and CPU cache hierarchies. The interesting problems live in the network: the space between machines where time is spent and where most of it is wasted.

## Starting From the Metal

To understand where time goes, start at the bottom.

A modern CPU runs at roughly 3-5 GHz. That's **0.2-0.3 ns per clock cycle**. A single instruction (an add, a compare, a branch) completes in one or a few cycles. A trivial program in a low-level language (C, Rust, Go), say, hashing a short string or parsing a small struct, runs in **microseconds**, well under a millisecond. In a high-level interpreted language (Python, Ruby), the same operation might be 10-100x slower due to interpretation overhead, dynamic dispatch, and memory allocation patterns, but even then, it's often still sub-millisecond for simple tasks.

Language choice still matters, but **raw compute for typical operations is almost never the bottleneck**. Even in a high-level language, the CPU finishes its work in a fraction of the time it takes a packet to cross a network. The moment networking enters the picture (an API call, a database query, a DNS lookup) compute becomes a rounding error next to data transfer. The optimization target shifts from "make the CPU faster" to "minimize the number of network hops and the distance each hop travels."

This is why region placement matters more than language benchmarks for most products, and why cross-region calls dominate latency budgets.

### The Human Speed Limit

There's also a ceiling on how fast latency needs to be, and it's set by the human on the other end.

The fastest typists hit **100-300 WPM**. That translates to roughly **10-30 ms per character** at the input layer. Average human reaction time (the time it takes to perceive a change and press a button) is around **200 ms**. Anything below that threshold feels instantaneous to the user. Anything below **10 ms** is imperceptible even for continuous interactions like typing or dragging.

Displays reinforce this. Most screens run at 60 fps (**~16.7 ms per frame**) or 120 fps (**~8.3 ms per frame**). No visual change can appear faster than the next frame. Even if your code responds in 1 ms, the user won't see the result until the display refreshes.

For a product, this means **optimizing a single action below the 10 ms window is almost never worth it**. An API responding in 8 ms instead of 3 ms is invisible to users. The returns on micro-optimization for individual operations are invisible.

The exception is **high-volume compounding operations**: things like batch processing, streaming data pipelines, or tight loops that execute thousands of times per request. A 1 ms improvement that compounds across 500 iterations is a 500 ms win. Optimize there. For single user-facing actions, respect the human speed limit and spend engineering time on the hops that actually matter.

## The Deceptive Simplicity of `fetch()`

A single `fetch('https://api.example.com/users')` in the browser looks instantaneous in code. But behind that single line lies a cascade of network operations, each contributing latency. Understanding where time is spent (and where it's wasted) is the difference between a snappy application and one that feels sluggish.

This post walks through the full request/response lifecycle from "user taps a button" to "UI updated," breaking down each phase with concrete numbers and identifying common footguns that can **double latency or worse**.

First, an anchor in reality: what latency should one actually expect?

## Baseline: Latency Within Your Infrastructure

Before browser-to-API calls, here are realistic expectations for latency within AWS/GCP/Azure infrastructure. These are rough ballparks, but the orders of magnitude are what matter.
...
There's no further optimization here for typical web workloads.
...
**This is where the latency budget gets killed under careless setups.**

### Is Further Optimization Worth It?

Once services and databases are in the same region (ideally same AZ), optimizing raw network latency further is almost never the first-order problem. The far bigger wins come from:
...
Obsessing over placement groups, kernel tuning, or shaving microseconds only makes sense for HFT, real-time audio/video at scale, or very high QPS low-latency trading/streaming systems.

## The Complete Request Lifecycle

Here's what actually happens when a user clicks a button and the browser executes `fetch()`:
...
Before the browser can connect to `api.example.com`, it must resolve that domain to an IP address.
...
A frontend that calls `api1.example.com`, `billing.example.com`, `realtime.example.com`, etc. forces each hostname to require its own DNS lookup, TCP+TLS handshake, and connection pool.

**Simple fix:**
- Use **one primary API hostname** per app (e.g., `api.example.com`) absent a concrete reason not to
...
- Avoiding 1-2 extra RTTs per new hostname saves **tens to hundreds of ms** on first page load
...
Once DNS resolves, the browser establishes a TCP connection via the three-way handshake:
...
Each HTTP request opens a new connection: TCP handshake (1 RTT) + TLS handshake (1 RTT). That effectively adds **2 RTTs** to every call.

**Simple fix:**
- Ensure **HTTP keep-alive** is enabled on the reverse proxy (NGINX/Envoy/API gateway) and app server
- Prefer **HTTP/2 or HTTP/3** between client and edge, and between edge and services; they multiplex multiple requests over one connection
...
Without TLS 1.3 and session resumption, every new connection pays the full handshake cost.
...
- Combined with keep-alive, TLS cost can compress to once per user session
...
**Rough rule of thumb:**
- Below **2-4 KB** header size, the bytes don't matter; extra round trips (redirects, auth challenges) matter more
...
This is where backend architecture decisions can easily add **50-300+ ms** (or more) under careless setups.
...
- For global users, use **edge routing** to send them to the nearest region containing the app and DB
...
On the backend (Go, Java, etc.), do the same with goroutines, futures, thread pools. Just **bound concurrency** to avoid overloading the DB or downstream services.
...
1. **Indexes:**
   ...
   - Use the DB's `EXPLAIN` to confirm indexes are in use, not full scans
...
**What to expect:**
- Fewer round-trips to the backend
- More importantly: the backend implementation can be **optimized once** (e.g., single set of DB queries)
...
- On server:
  ...
  - For lots of tiny responses, prefer `gzip` over `brotli` (less CPU) or skip compression for bodies < X bytes
...
- Consider **binary formats** for high-throughput APIs (Protobuf, CBOR, MessagePack); these can be 5x smaller
...
Total time for a typical browser request from NYC to us-east-1 (20ms RTT):
...
- Thousands of QPS with very small messages where every byte counts
- Strong typing and streaming semantics are required
...
The UI currently makes multiple REST calls and GraphQL folds them into a single, well-optimized backend query.

**GraphQL hurts latency when:**
Arbitrary queries are allowed, resolvers aren't batched or optimized, and the backend ends up doing many DB calls.
...
- Backpressure: If consumer is slow, the choices are buffer (memory), drop messages, or apply backpressure to producer
...
- Only the data RTT remains: **~10-20 ms**
...
## Where Latency Most Commonly Doubles

The top ways people accidentally 2-10× their latency:
...
Not every optimization is worth it. The order of operations for latency-sensitive work:
...
Focus on **P95 latency** (95th percentile), not averages. If P95 is 2 seconds, 5% of users have a terrible experience.

## Summary: The Biggest Wins

If only three things get fixed:

1. **Co-locate the app and DB in the same region (ideally same AZ)**: Saves 50-150 ms per avoided cross-region hop
...
These three changes alone often **halve API latency** for most users.

Latency is a stack of decisions, not a single number. Each phase compounds. Know where the time is going, fix the biggest offenders first, and measure the impact.
